...
jQuery(document).ready(function ($) {
    ...
    var importVersion = function (version, engine) {
        ...
        var data = {
            ...
            engine: engine,
            version: version,
            steps: ['reset', 'child_theme', 'engine', 'requirements', 'plugins', 'install', 'processing', 'final'],
            pageid: 0
        };
    ...
    function EtImportActiond(popup) {
        popup.find('.et_navigate-next').on('click', function (e) {
            ...
            if (step.next().hasClass('et_step-plugins') || step.next().hasClass('et_step-type')) {
                $('.et-mb-remove').remove();
            }
        ...
        popup.find('.et_navigate-install').on('click', function (e) {
            ...
            if (version=='coffee' || version=='restaurant'){
                _engine = 'elementor';
            }
    ...
    $('[href="https://wpkraken.io/?ref=8theme"]').attr('target', '_blank');
    $('[href="https://wpml.org/?aid=46060&affiliate_key=YI8njhBqLYnp"]').attr('target', '_blank');
